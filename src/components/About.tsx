...
import { 
  ...
  MapPin, Phone, Mail, Clock, ShieldCheck, ChevronLeft, ChevronRight, X, Compass, Info, ExternalLink
} from 'lucide-react';
          ...
          <div className="mt-20 pt-16 border-t border-stone-200/60">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {RECOMMENDATIONS.map((rec) => {
                const isExternal = Boolean(rec.externalUrl);
                return (
                  <div 
                    key={rec.id}
                    onClick={(e) => {
                      if (isExternal && rec.externalUrl) {
                        e.stopPropagation();
                        window.open(rec.externalUrl, '_blank', 'noopener,noreferrer');
                      } else {
                  ...
                  >
                    <div className="relative h-44 overflow-hidden bg-stone-100">
                      ...
                      <div className="absolute top-3 left-3 flex gap-2">
                        ...
                        {isExternal && (
                          <span className="text-[9px] font-sans uppercase tracking-wider bg-accent-terracotta text-white py-1 px-2 font-semibold rounded-md flex items-center gap-1">
                    ...
                    <div className="p-5 flex-grow text-left flex flex-col justify-between">
                      <div>
                        <h4 className="font-serif font-bold text-stone-900 text-base group-hover:text-accent-terracotta transition-colors duration-200 flex items-center justify-between">
                          <span>{rec.title[language]}</span>
                          {isExternal && <ExternalLink className="w-3.5 h-3.5 text-accent-terracotta shrink-0 ml-1" />}
                        </h4>
                      ...
                      <div className="mt-5 pt-3 border-t border-stone-200/50 flex items-center justify-between text-[10px] font-sans font-bold tracking-widest text-accent-terracotta uppercase">
                        <span>{isExternal ? (language === 'ca' ? 'Veure Torneig' : language === 'en' ? 'Open Tournament' : 'Ver Torneo') : (language === 'ca' ? 'Saber més' : language === 'en' ? 'Learn more' : 'Saber más')}</span>
                        {isExternal ? (
            ...
            >
              ...
              >
                ...
                <div className="md:w-7/12 p-6 md:p-8 flex-grow overflow-y-auto text-left flex flex-col justify-between">
                  ...
                  {/* Footer Back/Close & External Link */}
                  <div className="pt-6 mt-6 border-t border-stone-100 flex items-center justify-between gap-4">
                    {selectedRecommendation.externalUrl ? (
                      <a
                        href={selectedRecommendation.externalUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-5 py-2.5 bg-accent-terracotta hover:bg-accent-terracotta-hover text-white font-bold text-[10px] font-sans uppercase tracking-widest transition-colors rounded-lg flex items-center gap-2 cursor-pointer shadow-sm"
                      >
                        <ExternalLink className="w-3.5 h-3.5" />
                        <span>{language === 'ca' ? 'Obrir Web del Torneig' : language === 'en' ? 'Visit Tournament Site' : 'Abrir Web del Torneo'}</span>
                      </a>
                    ) : <div />}
                    <button
                      type="button"
